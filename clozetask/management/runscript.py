import os
import torch
import clozetask.management.modeling.model_setup as task_model_setup
import clozetask.management.runner as semeval_runner
import clozetask.management.metarunner as semeval_metarunner
import clozetask.management.components.evaluate as sem_evaluate
import clozetask.shared.distributed as distributed
import clozetask.shared.initialization as initialization
import clozetask.shared.model_setup as model_setup
import clozetask.utils.io as py_io
import clozetask.utils.zconf as zconf
import clozetask.management.components.container_setup as container_setup
...
def setup_runner(
    ...
    jiant_task_container: container_setup.TaskContainer,
    quick_init_out,
    verbose: bool = True,
) -> semeval_runner.TaskRunner:
    ...
    with distributed.only_first_process(local_rank=args.local_rank):
        # load the model
        jiant_model = task_model_setup.setup_model(
        ...
        task_model_setup.delegate_load_from_path(
    ...
    rparams = semeval_runner.RunnerParameters(
        local_rank=args.local_rank,
        n_gpu=quick_init_out.n_gpu,
        fp16=args.fp16,
...
def run_loop(args: RunConfiguration, checkpoint=None):
    ...
    with quick_init_out.log_writer.log_context():
        ...
        checkpoint_saver = semeval_runner.CheckpointSaver(
            metadata={"args": args.to_dict()},
            save_path=os.path.join(args.output_dir, "checkpoint.p"),
        )
        if args.do_train:
            metarunner = semeval_metarunner.JiantMetarunner(
                runner=runner,
        ...
        if args.do_val:
            ...
            sem_evaluate.write_val_results(
