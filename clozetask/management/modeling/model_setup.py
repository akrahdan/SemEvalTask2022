...
import clozetask.management.modeling.primary as primary
import clozetask.utils.strings as strings
from clozetask.management.modeling.heads import HeadFactory
from clozetask.management.modeling.taskmodels import TaskModelFactory, Taskmodel, MLMModel
...
def setup_model(
    ...
    encoder = primary.TransformersModelFactory()(hf_model)
    taskmodels_dict = {
...
def load_encoder_only(model, weights_dict):
    """Loads only encoder weights
    Args:
        model (SemEvalModel): a model (encoder and task models are core components).
        weights_dict (Dict): model weights.
    Returns:
        Dict[str, List] containing dropped keys
    """
    new_weights_dict = {}
    encoder_keys = [n for n, p in model.encoder.named_parameters()]
...
def load_partial_heads(
    model, weights_dict, allow_missing_head_weights=False, allow_missing_head_model=False
):
...
def create_taskmodel(task, encoder, **taskmodel_kwargs) -> Taskmodel:
    ...
    head = HeadFactory()(task, **head_kwargs)

    taskmodel = TaskModelFactory()(task, encoder, head, **taskmodel_kwargs)
    return taskmodel
