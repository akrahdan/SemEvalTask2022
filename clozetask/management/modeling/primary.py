...
import clozetask.utils.strings as strings
from clozetask.core import (
    BatchMixin,
    FeaturizationSpec,
    Task
)
from clozetask.management.components.outputs import construct_output_from_dict
from clozetask.management.modeling.taskmodels import Taskmodel
from clozetask.shared.model_resolution import ModelArchitectures

from clozetask.utils.tokenization_utils import (
    bow_tag_tokens,
    eow_tag_tokens,
    process_bytebpe_tokens,
    process_wordpiece_tokens,
...
def wrap_task_forward(
    semeval_model: Union[SemEvalModel, nn.DataParallel],
    batch: BatchMixin,
    task: Task,
    ...
    """
    assert isinstance(semeval_model, (SemEvalModel, nn.DataParallel))
    is_multi_gpu = isinstance(semeval_model, nn.DataParallel)
...
class TransformersModelFactory:
    ...
    def register(cls, model_arch: ModelArchitectures) -> Callable:
        ...
        def inner_wrapper(wrapped_class: TransformersModel) -> Callable:
    ...
    def __call__(cls, hf_model):
        ...
        Returns:
            TransformersModel: A wrapper class for Hugging Face model
        """
...
class TransformersModel(metaclass=abc.ABCMeta):
...
@TransformersModelFactory.register(ModelArchitectures.BERT)
class SemEvalBertModel(TransformersModel):
    def __init__(self, baseObject):
...
@TransformersModelFactory.register(ModelArchitectures.ROBERTA)
class JiantRobertaModel(TransformersModel):
    def __init__(self, baseObject):
        super().__init__(baseObject)
...
@TransformersModelFactory.register(ModelArchitectures.DEBERTAV2)
class SemEvalDebertaV2Model(TransformersModel):
    def __init__(self, baseObject):
        super().__init__(baseObject)
...
@TransformersModelFactory.register(ModelArchitectures.XLM_ROBERTA)
class SemEvalXLMRobertaModel(TransformersModel):
    def __init__(self, baseObject):
        super().__init__(baseObject)
...
@TransformersModelFactory.register(ModelArchitectures.XLM)
class JiantXLMModel(TransformersModel):
    def __init__(self, baseObject):
        super().__init__(baseObject)
...
@TransformersModelFactory.register(ModelArchitectures.ALBERT)
class JiantAlbertModel(TransformersModel):
    def __init__(self, baseObject):
        super().__init__(baseObject)
...
@TransformersModelFactory.register(ModelArchitectures.ELECTRA)
class SemEvalElectraModel(TransformersModel):
    def __init__(self, baseObject):
        super().__init__(baseObject)
...
@TransformersModelFactory.register(ModelArchitectures.BART)
class SemEvalBartModel(TransformersModel):
    def __init__(self, baseObject):
        super().__init__(baseObject)
    ...
    def encode(self, input_ids, input_mask, *args):
        ...
        return ModelOutput(pooled=pooled, unpooled=unpooled, other=other)

    def get_mlm_weights_dict(self, weights_dict):
        raise NotImplementedError()


@TransformersModelFactory.register(ModelArchitectures.MBART)
class SemEvalMBartModel(SemEvalBartModel):
    def __init__(self, baseObject):
        super().__init__(baseObject)
