...
import clozetask.management.runner as runner
import clozetask.management.components.task_sampler as task_sampler
from clozetask.shared.runner import (
    save_model_with_metadata,
    compare_steps_max_steps
)
from clozetask.utils.datastructures import ExtendedDataClassMixin
from clozetask.utils.torch_utils import copy_state_dict, CPU_DEVICE, get_model_for_saving
from clozetask.utils.zlog import BaseZLogger, PRINT_LOGGER
from clozetask.utils.functional import always_false
from clozetask.shared.metarunner import AbstractMetarunner

@dataclass
class ValState(ExtendedDataClassMixin):
    score: float
    metrics: Dict
    train_state: runner.TrainState
...
class JiantMetarunner(AbstractMetarunner):
    ...
    def eval_save(self):
        ...
        aggregated_major = task_sampler.compute_aggregate_major_metrics_from_results_dict(
            metrics_aggregator=self.runner.jiant_task_container.metrics_aggregator,
            results_dict=val_results_dict,
        )
        val_metrics_dict = task_sampler.get_metrics_dict_from_results_dict(
            results_dict=val_results_dict,
        )
        val_state = ValState(
